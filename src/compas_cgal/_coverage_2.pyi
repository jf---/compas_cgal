from collections.abc import Sequence
from typing import Literal
from typing import NoReturn

import numpy as np
import numpy.typing as npt

from compas_cgal._circle_geometry_2 import BoundaryNormalCircle2
from compas_cgal._circle_geometry_2 import BoundaryNormalCircleProposal2

Float64Array = npt.NDArray[np.float64]

class ReachableDomainConstructionError(RuntimeError): ...
class InvalidReachableDomainInputError(ReachableDomainConstructionError): ...
class ReachableArrangementTopologyError(ReachableDomainConstructionError): ...
class PocketNotMachinableError(ReachableDomainConstructionError): ...
class ReachableMaterialContainmentError(ReachableDomainConstructionError): ...
class ReachableMaterialPredicateGeometryError(ReachableDomainConstructionError): ...
class InvalidBoundaryCircleContactError(RuntimeError): ...
class BoundaryContactConstructionError(RuntimeError): ...
class InvalidCoverageGeometryError(RuntimeError): ...
class CoverageTransitionError(RuntimeError): ...

class WorldXYBoundaryPointMm:
    """Native exact world-XY millimetre contact; obtained from primitives or native circle proposals."""

    def __new__(cls, _native_only: NoReturn) -> WorldXYBoundaryPointMm: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    @property
    def reporting_xy_mm(self) -> tuple[float, float]:
        """Approximate reporting view; cannot be used as transition input."""
        ...

class ReachableBoundaryPrimitive2:
    def sample(self, parameter: float) -> WorldXYBoundaryPointMm: ...
    @property
    def start(self) -> WorldXYBoundaryPointMm: ...
    @property
    def end(self) -> WorldXYBoundaryPointMm: ...
    @property
    def kind(self) -> Literal["line", "arc"]: ...
    @property
    def start_mm(self) -> tuple[float, float]: ...
    @property
    def end_mm(self) -> tuple[float, float]: ...
    @property
    def source_piece_records(self) -> Sequence[bytes]: ...
    @property
    def arc_center_mm(self) -> tuple[float, float]: ...
    @property
    def arc_radius_mm(self) -> float: ...
    @property
    def arc_counterclockwise(self) -> bool: ...

class ReachableBoundaryCycle2:
    @property
    def counterclockwise(self) -> bool: ...
    primitives: Sequence[ReachableBoundaryPrimitive2]
    def ccw_transition(self, start: WorldXYBoundaryPointMm, end: WorldXYBoundaryPointMm) -> Sequence[ReachableBoundaryPrimitive2]: ...

def build_center_boundary_cycle(
    design_boundary: Float64Array,
    holes: Sequence[Float64Array],
    tool_radius: float,
) -> ReachableBoundaryCycle2: ...

class ExactRegion2:
    @staticmethod
    def from_polygon(boundary: Float64Array, holes: Sequence[Float64Array]) -> ExactRegion2:
        """Build the exact design region directly, without cutter offsets."""
        ...
    def clone(self) -> ExactRegion2: ...
    def contains(self, x: float, y: float) -> bool: ...
    def is_empty(self) -> bool: ...
    def component_count(self) -> int: ...
    def is_subset_of(self, other: ExactRegion2) -> bool: ...
    def exactly_equals(self, other: ExactRegion2) -> bool: ...
    def arrangement_traits_are_owned_for_audit(self) -> bool:
        """True when this region keeps alive the ReachSet owning the traits its arrangement reads."""
        ...

def remaining_material(
    target: ExactRegion2,
    circles: Float64Array,
    segments: Float64Array,
    disks: Float64Array,
    tool_radius: float,
) -> ExactRegion2:
    """Exact final residual, without accumulated sweeps or chronological history.

    World-XY millimetres: circles Nx3=(cx,cy,guide radius), segments
    Nx4=(x0,y0,x1,y1), disks Nx2=(cx,cy). Segment endpoints must differ.
    Every shape/value is validated before geometry, including unused tails.
    """
    ...

class ReachableDomainCertificate2:
    @property
    def strategy_version(self) -> bytes: ...
    @property
    def source_curve_records(self) -> Sequence[bytes]: ...
    @property
    def arrangement_vertex_records(self) -> Sequence[bytes]: ...
    @property
    def selected_cell_records(self) -> Sequence[bytes]: ...
    @property
    def component_records(self) -> Sequence[bytes]: ...
    @property
    def exact_cell_selection(self) -> bool: ...
    @property
    def complete_source_provenance(self) -> bool: ...
    @property
    def reachable_subset_of_design(self) -> bool: ...
    def matches_exact_inputs(
        self,
        boundary: Float64Array,
        holes: Sequence[Float64Array],
        tool_radius: float,
    ) -> bool: ...

class ReachableDomain2:
    def __init__(
        self,
        design_boundary: Float64Array,
        holes: Sequence[Float64Array],
        tool_radius: float,
    ) -> None: ...
    def design_region(self) -> ExactRegion2: ...
    def circle_on_piece(self, piece_index: int, parameter: float, tool_radius: float) -> BoundaryNormalCircleProposal2:
        """One-sided source query; arc/segment competitor indices name original curves."""
        ...
    def center_domain(self) -> ExactRegion2: ...
    def reachable_material(self) -> ExactRegion2: ...
    def unreachable_residual(self) -> ExactRegion2: ...
    def certificate(self) -> ReachableDomainCertificate2: ...

class CutterCentreDomain2:
    @classmethod
    def build(
        cls,
        design_boundary: Float64Array,
        holes: Sequence[Float64Array],
        tool_radius: float,
    ) -> CutterCentreDomain2: ...
    def contains(self, x: float, y: float) -> bool: ...

class ReachableMaterialPredicate2:
    @classmethod
    def build(
        cls,
        design_boundary: Float64Array,
        holes: Sequence[Float64Array],
        tool_radius: float,
    ) -> ReachableMaterialPredicate2: ...
    def contains(self, x: float, y: float) -> bool: ...

class CoverageSweepRecord2:
    @property
    def strategy_version(self) -> bytes: ...
    @property
    def structural_record(self) -> bytes: ...
    @property
    def segment(self) -> bool: ...
    @property
    def center_x(self) -> float: ...
    @property
    def center_y(self) -> float: ...
    @property
    def first_x(self) -> float: ...
    @property
    def first_y(self) -> float: ...
    @property
    def tool_radius(self) -> float: ...
    def matches_exact_segment(
        self,
        x0: float,
        y0: float,
        x1: float,
        y1: float,
        tool_radius: float,
    ) -> bool: ...
    def matches_exact_full_circle(
        self,
        center_x: float,
        center_y: float,
        phase_x: float,
        phase_y: float,
        tool_radius: float,
    ) -> bool: ...

class Coverage2:
    @staticmethod
    def from_uncut(target: ExactRegion2) -> Coverage2:
        """Replay from an uncut design or reachable-material target."""
        ...
    def add_disk_sweep(self, center_x: float, center_y: float, tool_radius: float) -> None:
        """Subtract an actual stationary cutter disk or plunge footprint."""
        ...
    def __init__(
        self,
        reachable_material: ExactRegion2,
        precleared_x: float,
        precleared_y: float,
        precleared_radius: float,
    ) -> None: ...
    def clone(self) -> Coverage2: ...
    def add_segment_sweep(
        self,
        x0: float,
        y0: float,
        x1: float,
        y1: float,
        tool_radius: float,
    ) -> CoverageSweepRecord2: ...
    def add_full_circle_sweep(
        self,
        center_x: float,
        center_y: float,
        phase_x: float,
        phase_y: float,
        tool_radius: float,
    ) -> CoverageSweepRecord2: ...
    def residual_is_empty(self) -> bool: ...
    def residual_component_count(self) -> int: ...
    def residual(self) -> ExactRegion2: ...
    def accumulated_sweeps(self) -> ExactRegion2: ...
    @property
    def residual_component_records(self) -> Sequence[bytes]: ...
    @property
    def sweep_records(self) -> Sequence[bytes]: ...
    def exact_residual_relation(self) -> bool: ...
    @property
    def strategy_version(self) -> bytes: ...

def boundary_circle_contact(proposal: BoundaryNormalCircleProposal2) -> WorldXYBoundaryPointMm:
    """Preserve the exact native proposal contact for boundary transitions."""
    ...

def boundary_circle_at_contact(owner: BoundaryNormalCircle2, contact: WorldXYBoundaryPointMm, tool_radius: float) -> BoundaryNormalCircleProposal2: ...

class InvalidNativeBoundaryCurveError(RuntimeError): ...
class InvalidNativeBoundaryChainError(RuntimeError): ...

class NativeBoundaryCurve2:
    """Exact endpoint-preserving geometry; no G1 or source-fit certification."""
    @staticmethod
    def line(start: tuple[float, float], end: tuple[float, float]) -> NativeBoundaryCurve2: ...
    @staticmethod
    def arc(start: tuple[float, float], end: tuple[float, float], center: tuple[float, float], counterclockwise: bool) -> NativeBoundaryCurve2: ...
    @property
    def start(self) -> WorldXYBoundaryPointMm: ...
    @property
    def end(self) -> WorldXYBoundaryPointMm: ...
    @property
    def is_arc(self) -> bool: ...
    @property
    def center_adjustment_mm(self) -> float:
        """Approximate displacement report, not a trimmed-source error bound."""
        ...
    @property
    def center_mm(self) -> tuple[float, float]: ...
    @property
    def radius_mm(self) -> float: ...

class InvalidNativeBoundaryMedialInputError(RuntimeError): ...
class NoPositiveNativeBoundaryCircleError(RuntimeError): ...
class NativeBoundaryMedialConstructionError(RuntimeError): ...

class NativeBoundary2:
    def __init__(self, curves: Sequence[NativeBoundaryCurve2]) -> None: ...
    @property
    def curves(self) -> Sequence[NativeBoundaryCurve2]: ...
    @property
    def cycle(self) -> ReachableBoundaryCycle2: ...
    def design_region(self) -> ExactRegion2: ...
    def circle_on_piece(self, piece_index: int, parameter: float, tool_radius: float) -> BoundaryNormalCircleProposal2:
        """One-sided source query; arc/segment competitor indices name original curves."""
        ...
