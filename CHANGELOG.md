...
### Added
- `stock` / `engagement`: exact planar stock model (`Stock2`, Epeck + circle-segment traits) plus a tool-engagement-angle (TEA) certifier. `engagement_at` returns exact per-station TEA and an exact cap verdict; `certify_segment_tea` and its arc mirror certify the cap along linear/circular cut motions with guarded exact thresholds (Monte-Carlo-validated against a raster oracle). All cap decisions are exact predicates on the exact arrangement; `to_double`/`atan2` are reporting-only.
- `engagement`: gap-closure pessimism closes a certificate merge hole — where two engaged runs fuse across a thin closing rim void, `max_run` jumps O(1), unbounded by the O(√d) growth lemma; each station now pre-absorbs void gaps ≤ γ_guard = 2·GROWTH(hs) (exact chord/orientation predicates, no angle sums) so a merge completing within a step cannot slip past the cap decision. Closed for both linear and circular motions and regression-guarded; reported TEA stays the true measure.
- `engagement`: `audit_toolpath_engagement` — per-op TEA report (frozen dataclasses) replaying a toolpath through the sound certifier against a cap. `scripts/engagement_baseline.py` is the SP2-reference audit harness; full generation is O(n²) exact stock depletion and is deferred (see `docs/superpowers/state/sp1-gate-c-analysis.md`).
