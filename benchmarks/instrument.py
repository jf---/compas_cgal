...
Both probes read the state a stock is already in; reaching a depleted state is
`benchmarks.depletion`'s job, and the two are always used together -- a probe on
an undepleted stock reports the virgin values and can never move.
"""
...
def probe_digits(stock: Stock) -> CoordinateDigits:
    ...
    collapses the lazy-exact filter and inflates every later operation on the
    same stock.
