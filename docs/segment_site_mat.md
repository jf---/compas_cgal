...
!!! warning "Implementation maturity"
    ...
    native fields 1 and 16–18. Field 19 now rebuilds Task 3's exact reachable
    domain from the same canonical input and reproduces the Python
    `ReachableDomainCertificate` CCAN bytes and center-domain digest exactly.
    Field 20 now seals that digest with the exact MAT graph, clearance
    profiles, dense integer projection, and neck evidence/cuts in one
    refinement-invariant CCAN certificate with exact native replay.
    The same proof owner now retains a sidecar inventory for edges whose
    complete exact clearance polynomial is identically the tool-radius square.
    The radius-1 L fixture emits exactly two replayable zero-guide records;
    Python projects them into immutable MAT-digest-bound proof records and a
    finite spatial-only link-candidate family. A typed advancing-segment
    operation and closed physical state grammar now consume that candidate
    shape without pretending it is a circle. Atomic transaction and generator
    ...
    width. A new catalog-fed L bundle now returns the unchanged canonical graph
    plus one such profile per retained edge from one graph build. Its exact
    profile decomposition is two constants, five quadratics, and two quartics
    at `r² = 0`; input reversal preserves every coefficient and endpoint, and
    positive-radius clearance endpoints re-evaluate to `width² = 4`.
    A graph-level typed classifier now combines those profiles with one reused
    cut index. Synthetic goldens cover strict interior, single clearance-root
    endpoint, merged shared-vertex, and maximal connected plateau ownership.
    ...
    Task 13B now consumes those sides through one deterministic nine-edge L
    route, retains exact visited incidences and per-edge cursors, and derives a
    unique oriented transit only when the route leaves a locus for a different
    certified side.
    ...
    Exact segment/full-circle continuous engagement now reaches the typed
    `MotionWitness` boundary. Task 11A independently reconstructs a no-neck
    two-circle prefix and its derived-cursor link, rejects invalid pairing
    before stock mutation, and proves both circles plus the link against their
...
## Task 10 typed planning boundary

Task 10 turns the low-level Task 9 producer into the first planner-facing
domain model. Tasks 12–13B extend it to four layers with deliberately
different authority:

| Layer | Owner | Authority | Explicitly not authoritative for |
| --- | --- | --- | --- |
| Exact MAT proof | native `SegmentSiteMedialAxis` | exact graph, catalog, site/node/edge/dual/parameter identity, neck evidence, complete zero-guide inventory, center-domain digest, and MAT-certificate replay | planner policy or mutable traversal state |
| Typed planning state | Python `MedialAxis`, `MatZeroGuideInventory`, `NeckInventory`, and `NeckPassage` | immutable topology/proof views, exact identity references, verified neck class, oriented passage transition, and bound effective cap | geometric acceptance of a proposed motion |
| Finite proposal lattice | Python `MiddleCurveSpan`, `MiddleCurveCandidate`, and `ZeroGuideLinkCandidate` | complete deterministic circle or proved-link proposal enumeration and canonical winner order | gouge freedom, engagement, depletion, or coverage |
| Global route state | Python `TraversalSampleIndex`, `TraversalGraph`, `MatTraversalAuthority`, and `MatTraversalState` | refinement-dependent cursor inventory, deterministic directed edge route, visited incidences, causal neck sides, and one active cursor | mutable stock/coverage, motion acceptance, or terminal residual proof |

This is an authority split, not four implementations of the MAT. Native code
...
`SegmentSiteMatBundle2` owns the exact `MatProposalSamplingGraph2`, canonical
site catalog, center-domain digest, numeric table, and the exact identities
that the fixed Task 9 tuple intentionally does not carry. The Python-bound
`SegmentSiteMedialAxis` owns that bundle.

The existing `segment_site_medial_axis(...)` function and
`SegmentSiteMedialAxis.projection` are detached views of this same owner. The
function therefore remains compatible with the frozen 20-field producer
contract without creating a second build path. Mutating a returned NumPy
projection cannot mutate the proof owner; asking for a new projection returns
the original values.

Exact identity dictionaries are exposed separately from the numeric tuple:

- canonical site, node, edge, and original-dual bytes;
- exact per-sample parameter bytes;
- canonical neck-owner and defining-site bytes; and
- canonical zero-guide edge/certificate pairs.

`MedialAxis.build(...)` invokes the native owner once and validates the tuple's
ranks, dtypes, cardinalities, CSR terminals, exact-verdict flags, and world-XY
reporting geometry. It then constructs:

- typed `MatSite`, `MatNode`, and `MatEdge` records using the native identity
  dictionaries directly;
- stable connected-component and edge-granular branch identities;
- one maximal tool-fit run per exactly clipped parent edge, retaining both
  endpoint-provenance states;
- proposal samples whose cursor identity hashes the exact edge and exact
  parameter identities;
- one `MatProof` that keeps the native owner, center-domain digest, MAT
  certificate, and typed zero-guide inventory alive together.

No coordinate match participates in topology or identity. Coordinates are
reporting/proposal data only.

!!! danger "The fixed tuple cannot become a second proof owner"

    The 20-field Task 9 projection omits several exact identity dictionaries
    by design. Reconstructing them by matching doubles would make reporting
    geometry decide topology; reparsing certificate bytes in Python would
    create an alternate proof implementation; rebuilding the MAT would
    duplicate the dominant exact work. Retain the native owner and derive
    every view from that single call graph.

### Native exact zero-guide proof inventory

For a complete clipped edge interval with exact squared-clearance polynomial
`C_e(t)` and exact tool-radius square `r²`, the native predicate is:

```text
zero-guide(e, r) iff coefficients(C_e) == [r²]
```

This is polynomial identity over `CORE::BigRat`, not equality at endpoints or
proposal stations. A nonconstant profile that touches `r²` at one endpoint is
not a zero-guide run; neither is a constant whose binary64 radius is merely
near the required value.

`SegmentSiteMatBundle2` retains the typed tool radius and constructs the
complete `MatZeroGuideRecordV1` inventory once from its owned
clearance-profile graph. Each record canonically binds:

- the complete MAT-certificate digest and center-domain digest;
- the full clearance-profile record, including edge, defining sites, exact
  endpoint-root identities, and every coefficient;
- the exact tool-radius square;
- the versioned coefficient-identity strategy; and
- the positive identity verdict.

The owner exposes sorted immutable `(edge_id, record_bytes)` pairs outside the
frozen 20-position numeric projection. Its explicit verifier first replays the
submitted MAT certificate against the retained graph/catalog/domain owner,
then reconstructs the complete expected inventory and compares edge ownership
and bytes exactly. Missing and duplicate ownership have distinct named errors;
mutated or foreign bytes raise the named mismatched-record error. An ordinary
nonmember produces no record.

At radius `1`, the adopted L fixture proves exactly two constant-clearance
width-2 S–S arms. Repeat construction and canonical ring reversal reproduce
both records byte for byte. Radius `0.5` produces none, as does
`nextafter(1, 0)`; the P–S edge whose clearance only touches the tool radius at
an endpoint also remains absent. Task 1 therefore closes native membership and
replay authority.

### Typed zero-guide proof projection

`MedialAxis.build(...)` now obtains the retained native record tuple and asks
`SegmentSiteMedialAxis.validate_zero_guide_records(...)` to replay it under the
complete MAT certificate before constructing any Python run. The returned
edge sequence must equal the record sequence exactly. Only then does
`MatZeroGuideInventory.build(...)` create sorted immutable `MatZeroGuideRun`
records. Each run binds:

- its exact `MatEdgeId`;
- the `IdentityDigest` of the complete MAT certificate; and
- the untouched canonical native certificate bytes.

`MatProof` retains this inventory beside the native owner. Its constructor
requires exact equality with the owner's retained MAT certificate and
zero-guide records; `MedialAxis` separately requires every proved edge to
exist in its typed topology. The derived
`MedialAxis.zero_guide_run_by_edge_id` view is read-only and has no independent
mutable authority. Cross-wiring an edge ID, one record byte, the MAT digest,
or the native owner raises `InvalidZeroGuideCertificateError`.

!!! tip "Captured implementation insight: replay once, bind thereafter"

    Exact replay belongs at the native-to-Python projection boundary. Replaying
    again inside every aggregate constructor would duplicate expensive exact
    work; trusting only Python structure would weaken authority. The balanced
    contract performs one complete native replay, then makes raw construction
    and `dataclasses.replace` bypass-safe by comparing the typed records,
    certificate, and digest against the immutable retained owner. Python never
    parses a proof payload or reclassifies reporting floats.

Task 2 closes typed Python ownership and Task 3 closes finite link-candidate
construction. Advancing operation and transaction construction, route-1
consumption, terminal residual, and final replay remain subsequent tasks.

### Exact neck replay before mutable passage state

`SegmentSiteMedialAxis.validate_and_classify_necks(...)` first replays the MAT
certificate against the retained exact graph, site catalog, and center-domain
identity. Only then does it validate the supplied `NeckEvidenceV1` bytes and
compare each algebraic `width²` to the policy's canonical
`ExactRationalV1` boundaries. It returns an integer class and a comparison
certificate for every evidence record.

`NeckInventory.build(...)` binds each result to:

- its canonical neck-owner identity;
...
```text
unvisited -> first_pass_complete -> second_pass_complete -> terminal
```

`propose_cap_decision(...)` records the evidence digest, width class, before
and after states, user cap, and policy-selected effective cap.
`advance(...)` accepts only that neck's next legal transition. Terminal
passages fail with `TerminalNeckPassageError`.

### Directed global traversal owns causal neck scope

Task 13B separates static route authority from evolving traversal state.
`TraversalSampleIndex` scans the native proposal samples once and retains
immutable edge and cursor lookup views. Its canonical record binds every
sample's exact parameter identity, cursor identity, edge identity, and ordinal.
`MatTraversalAuthority` also binds station spacing, sagitta bound, and
refinement depth directly. Two sampling policies may therefore share the same
refinement-invariant MAT certificate and `TraversalGraph`—and may even realize
the same sample index—while still producing different traversal-authority
digests.

`TraversalGraph` projects exact component, edge, branch, node, and native
endpoint-cursor identities. It assembles component and node incidence in one
edge pass, applies `TraversalPolicy` only at deterministic ordering
boundaries, and performs iterative depth-first discovery. The route covers
every edge exactly once, retains both incidences of a cycle edge, visits the
authenticated entry component first, and never uses reporting coordinates.
Synthetic chain, branch, cycle, multi-component, and relabelled-node contracts
exercise this boundary; the production L fixture seeds one directed cursor
for each of its nine edges.

`MatTraversalAuthority` derives the sample index, graph, route, and complete
neck-frontier schedule once from the exact MAT owner, neck inventory, policy,
and authenticated entry. `MatTraversalState` then carries only evolving facts:

- one native or proof-carrying cursor per directed route edge;
- the active route index;
- every visited `(edge, node)` incidence;
- one exact side position per classified neck; and
- at most one pending `CausalNeckTransit`.

Every child retains the same authority object. One accepted candidate advances
exactly the active cursor, rejects stale or foreign cursor lineage, consumes
the pending neck transit, and leaves all other cursors byte-identical. Branch
activation requires the current edge to be terminal; the final state requires
every cursor and both incidences of every edge to be complete.

`resolve_neck_side(...)` accepts only one exact side or the exact neck locus.
Overlapping side partitions fail even when their flattened union is unchanged,
and one route activation may not merge transitions for two independently
active necks. A transit binds its source and target `NeckSide` identities,
derives orientation from their canonical order, and selects the unique
matching `NeckPassage`.

!!! warning "A locus edge is not a side transition"

    While the route occupies an exact neck locus, retain the last certified
    side. Attribute the crossing only to the first later edge owned by a
    different certified side. Advancing passage state at the locus would fire
...
!!! warning "Proposal samples never classify a neck"
    ...
    `sample_clearance` and station spacing are reporting/proposal controls.
    Neck ownership, squared width, and policy class come from replayed exact
    evidence. Refining the sampler may change the number of candidate stations
    but cannot change the neck inventory or its comparison certificates.
...
`DerivedCandidateCursor` retains the exact parent `MiddleCurveSpan` and closed
`TraversalCandidate`. Both variants replay component, edge, branch,
cursor-before, cursor-limit, progress, target, and cursor-after identity. A
circle candidate additionally proves generator-site ownership; a zero-guide
candidate proves that its complete native record is the run owned by the same
MAT edge. The derived point can then open a new span whose native limit is the
...
directrix coordinate and re-evaluates the focus/directrix equation; it never
uses the sample chord as the curve. The dyadic L-fixture golden requires the
true parabolic midpoint `(1.75, 1.015625)` and rejects the chord midpoint
`(1.75, 1.03125)`.

!!! warning "Refine the MAT parameter, not the sample chord"

    Connecting two valid parabolic MAT samples by a line produces off-MAT
    middle points even though both endpoints and every later containment test
...
1. At spatial level `i in [0, L_s)`, let `s_i = s / 2^i`. Emit every positive
   multiple `k s_i` that is at least `delta_min` and below `L`, then emit `L`.
2. Merge duplicate spatial values and retain every level that produced each
   value.
3. At every station `m` and for each of the edge's two exact generator sites,
   compute the reporting boundary footpoint `p`, tool-contact point
   `q = p + r (m-p)/|m-p|`, and maximum one-sided MATHSM radius
   `rho_max = (|m-p|-r)/2`.
4. At radius level `j in [0, L_h)`, let `h_j = h / 2^j`. Emit
   `rho_min + k h_j` below `rho_max`, plus `rho_max`; merge duplicates while
   retaining all producing levels.
5. For every retained radius, emit every phase index in `[0, P)`.

The maximum-radius, zero-phase lane is the paper's one-sided MATHSM circle:
`c = midpoint(m,q)` and phase vector `q-c`. A smaller-radius lane retains the
same one-sided tool contact `q`, places its center at that radius toward `m`,
and uses `q-c_rho` as its zero phase. Other finite phase choices rotate this
base vector. These rotations and the interpolated middle point are
deterministic binary64 proposals; later exact containment and continuous
engagement gates must accept them independently.

The circle implementation exposes no feasibility predicate and performs no
bisection. It materializes the declared Cartesian set, then orders the closed
circle/link union by:

1. furthest exact progress;
2. largest squared candidate radius, with an exact link represented by zero;
3. canonical candidate identity.

Candidate identity seals the proposal geometry, generator site, full policy,
spatial/radius level provenance, cursor limit, neck scope, effective-cap
decision, and proposed `AdvanceTraversalDecision`. The small constant-clearance
golden independently enumerates `3 spatial x 3 radius x 4 phase x 2 site = 72`
cells. Repeated enumeration is byte-identical. A mutation that removes the cap
decision makes first- and second-passage identities collide and is killed by
the test.

#### Exact zero-guide link lattice

`enumerate_zero_guide_link_candidates(...)` first requires the span edge to
resolve in `MedialAxis.zero_guide_run_by_edge_id`. An absent run raises
`UncertifiedZeroGuideEdgeError`; proposal clearance, endpoint contact, and
binary64 equality are not alternate admission paths. For an owned run, the
enumerator reuses the exact same merged spatial values, curve interpolation,
cursor identity, neck scope, cap decision, and `AdvanceTraversalDecision` as
the circle family. It emits one `ZeroGuideLinkCandidate` per spatial value,
including the native limit, with no site, radius, or phase loop.

The candidate's distinct `zero-guide-link-candidate-v1` record binds the exact
edge, complete MAT-certificate digest, untouched native proof bytes, complete
candidate policy, spatial level provenance, target, cursor limit, causal
scope/cap, and traversal decision. It deliberately has no `guide_radius`,
`phase_index`, or `motion`. Both certified three-millimetre L arms emit exactly
`12` candidates under the focused policy in either direction. Repeat order is
byte-identical and forward/reverse identities are disjoint.

`TraversalCandidate` is the explicit union of circle and zero-guide records.
At equal progress, the common order ranks a positive-radius circle before a
zero-guide link. A terminal zero-guide candidate must reach its owned native
endpoint; only the positive-radius family may create an
`ExhaustedCandidateCursor` before that endpoint.

!!! tip "Captured implementation insight: proof collapses the product"

    Once exact polynomial identity proves a complete zero-guide edge, proposal
    work collapses from `spatial x site x radius x phase` to `spatial`. This is
    not a zero-radius circle: there is no nonzero circle phase vector, no site
    choice, and no radius search. Keeping a distinct candidate type both
    preserves the theorem and removes irrelevant branching. The `12`-record
    fixture count is a structural performance result; it is not yet a matched
    Held–Pfeiffer planner timing.

#### Advancing segment operation grammar

`AdvanceSegmentOperation` is the only physical operation that advances a MAT
cursor using a segment without a successor circle. Its distinct
`advance-segment-v1` record binds `ExactSegmentMotion`, `CutZ`, `NeckScope`,
the matching `EffectiveCapDecision`, and one `AdvanceTraversalDecision`.
Malformed construction raises `InvalidAdvanceSegmentOperationError`; it does
not escape through the older generic operation-identity boundary. A
`LinkSegmentOperation` remains a hold operation and its canonical bytes are
unchanged.

The authoritative physical state accepts exactly:

```text
entry circle ((hold link, full circle) | advancing segment)*
```

Every lateral motion must still occur at the qualified `CutZ` and appear in
the exact stock and coverage lineages. A hold link begins at the accepted
phase and must be followed immediately by a circle with identical scope, cap,
and cursor-before ownership. An advancing segment begins at the accepted
phase, updates that phase to its endpoint, owns its traversal advance, and may
terminate the state. Both advancing variants consume an oriented neck-cap
transition; a hold link never does. The final phase and physical cursor must
equal the final advancing operation.

!!! note "Captured insight: physical chronology does not own MAT branch switching"

    `GenerationState` proves each physical operation group's internal
    chronology. `TraversalCommit` separately authenticates transitions between
    MAT routes. Requiring consecutive physical advances to retain one edge or
    branch would reject a legitimate, already-authenticated branch switch and
    duplicate graph authority inside the machining stream. Keep link/circle
    pairing local, final-state binding global to the physical prefix, and
    route-transition proof in the traversal ledger.

#### Atomic advancing-segment proof

Task 5 closes the physical consumer for one proved zero-guide candidate.
`ZeroGuideLinkTransaction` binds the candidate, one
`AdvanceSegmentOperation`, containment, motion, depletion, coverage,
traversal/passage result, parent state, and child state under one canonical
record. Evaluation uses trial-local stock and coverage; commit independently
replays the complete evaluation and requires byte identity.

The TEA proof is intentionally not the frozen-stock segment oracle used by a
hold link. For a translation from an exactly clear start disk, every strictly
trailing rim point at `t > 0` lies inside a sufficiently close earlier cutter
disk. Only the closed forward semicircle can remain engaged, proving an exact
`pi` bound. The native producer returns a sealed,
self-reconstructing `SweptPrefixSegmentTeaAudit2` that binds the motion, cap,
exact stock-boundary digest, strategy version, theorem version, verdict,
start-clear and exact-`pi` decisions, and two motion strata. Python projects it
into the distinct `SweptPrefixMotionWitness`; an ordinary `MotionWitness`
cannot be relabelled into this proof family. The planning `InputIdentity` binds
the strategy and theorem as separate components from the generic event oracle
and witness schema, so either form of proof drift changes the root replay
identity. An uncleared start or tighter cap stays unresolved.

!!! tip "Captured insight: engagement follows the evolving cut, not frozen stock"

    A stationary cutter query against the original stock is not the physical
    state of an advancing slot cut. Treating its unresolved boundary reference
    as a numeric pathology would repair the wrong theorem. The correct proof
    explicitly consumes the cutter's swept prefix, while the independent
    capsule-subset certificate still owns gouge safety. Operation type selects
    the theorem; there is no fallback between them.

!!! tip "Captured insight: structural validation and authentication differ"

    A coverage parent lineage can have the correct tuple shape while carrying
    a foreign same-length digest. The transaction constructor can validate
    chronology and types but cannot recover that digest's preimage. Independent
    commit replay owns the parent state, reconstructs the real lineage, and
    rejects the canonical-byte mismatch. Keeping those responsibilities
    separate avoids a false local proof claim.

!!! tip "Captured insight: sealed proof records defeat semantic relabelling"

    A SHA-256 digest authenticates a byte string, not the meaning claimed for
    it. Searching those bytes for expected fragments likewise does not prove a
    unique schema. The native swept-prefix audit therefore reconstructs and
    verifies its complete record, and the Python boundary carries it in a
    dedicated witness type. Replay rejects both mutated proof fields and a
    structurally plausible generic witness relabelled as an advancing cut.

This closes atomic segment consumption. Task 6 now routes the global generator
through that transaction; fresh replay and terminal residual proof remain the
next gates.
...
### Atomic candidate transaction boundary

Task 12 now turns one finite-lattice circle into a joint direct-link/full-circle
transaction without exposing trial mutation. `GenerationState` owns independent
exact stock and coverage snapshots, the current world-XY phase, one typed MAT
cursor, every oriented-neck passage, and the accepted operation prefix. Its
SHA-256 identity binds all of them. Construction replays stock/coverage
chronology, lateral cut-depth continuity with the qualified entry, cursor and
phase continuity, link/circle scope-cap-cursor pairing, and every neck
transition from `UNVISITED`; a locally valid but unearned passage endpoint is
not authoritative state.

Before native work, `CandidateEvaluator` binds the candidate to the declared
lattice policy, validates material-side orientation against cut intent, and
rechecks the parent's depletion and cap decisions against the invariant
policies. It then forks stock and coverage and applies one order to each motion:

```text
exact containment -> event-exact TEA -> exact depletion -> exact coverage
```

The link observes parent stock. The circle observes the post-link trial stock.
If link containment or engagement fails, no circle query runs. If the circle
gouges, exceeds its effective cap, or remains unresolved, the already-proved
link is discarded with the trial owners. There is no rollback path because
authoritative state was never mutated.

Success returns a frozen `CandidateTransaction`. Its canonical bytes bind the
parent digest, finite candidate, consecutive link/circle operations, both
containment/motion/depletion/sweep bundles, certify-before-deplete lineage,
result cursor and passage, and child-state digest. Explicit commit first
rejects a stale parent, then independently repeats the winning evaluation and
requires byte-identical transaction evidence before returning the recomputed
child. Losing trials never own authoritative state; the one winner pays one
additional bounded evaluation.

The stage-close fixture is the real concave L pocket rather than a mock
geometry. Its 29 focused contracts include a legal direct link followed by the
production `80 degrees` circle-cap rejection, a real full-circle gouge,
injected named link and unresolved-circle failures, successful two-owner
commit, stale-parent rejection, cut-plane drift, unearned/discontinuous passage
state, cross-wired evidence/history, reordered chronology, authoritative cursor
mismatch, foreign candidate/depletion/cap/cut-direction policy, and
permutation-invariant winner selection.

!!! note "Exact owner identity is proof provenance"

    A derived cursor must retain the same `MedialAxis` owner that produced its
    candidate. Reconstructing a geometrically equal axis and rematching by
    coordinates is not equivalent: it detaches the candidate from its exact
    span lineage. Task 12's test fixture therefore constructs each candidate
    family once and reuses that owner through all continuations. Production
    Task 13B preserves the same owner through its sample index and cursor
    ledger.
...
`InputIdentity`. Tasks 6–8 now also certify an individually submitted segment
or full circle against its exact effective engagement cap and return a typed,
authority-bound `MotionWitness`. Task 11A now composes those contracts for a
freshly reconstructed no-neck L-pocket prefix: qualified entry depletion,
first-circle certification/depletion/coverage, relational link/circle
preflight, and exact link certification/depletion/coverage execute in proof
order. The following nonuniform circle now also receives an exact
cell-decision authority, certifies against frozen post-link stock, and only
then depletes stock and adds coverage. Replay subsequently fails at the
...
Later Task 13 stages own those traversal/selection obligations, Task 14 owns
artifact assembly, and Tasks 14A–16 own downstream validation, benchmarking,
and release evidence.

## How to assess the implementation's sophistication

The implementation is best described as **research-lab-grade proof-carrying
computational geometry for CAM**. That description is deliberately narrower
than “production CAM system” and stronger than “geometry prototype.”
Sophistication is uneven across dimensions:

| Dimension | Current level | Evidence | Remaining boundary |
| --- | --- | --- | --- |
...
The unusual difficulty is compositional. MAT topology, algebraic geometry,
machining policy, mutable stock, engagement, coverage, and provenance each
have a separate deciding authority. Correctness requires their records to
agree in one order-sensitive execution without letting a sampled coordinate,
producer snapshot, or already-mutated stock substitute for the fact being
proved. Repository size is a poor proxy for that difficulty.

!!! note "Exact and proof-carrying are not formally verified"

    Exact kernels remove numerical ambiguity from the declared constructions
    and replay checks make many cross-layer substitutions falsifiable. They do
    not constitute a machine-checked proof of the complete programme. Claims
    remain bounded by the implemented producers, closed failure modes, and
    committed adversarial fixtures.

## Tasks 6–8 exact continuous engagement

The exact segment and full-circle motion oracles now share one typed consumer
and one stationary branch/pair theorem without sharing a weaker proof
authority. A segment verdict consumes the larger `SegmentEventPartition2`,
including exact motion strata, active branch order, and pair-cap dispositions.
A nonuniform circle first reconstructs its generic four-chart event partition,
then maps one exact rational witness per open cell back to an owned quarter
chart, reconstructs the stationary stratum, and classifies the cyclic material
runs. `EventTrace2` v2 retains the generic certificate for common consumers
while hashing the complete deciding record as `decision_authority_digest`.

The segment producer also performs exact circular-support reach pruning before
quadratic pair-resultant construction. The recovery command fell from more
than 176 seconds unfinished to 39.42 seconds complete after the combined
tangent-root and reach-pruning repair. This is a bounded development
observation, not a matched planner benchmark.

The full-circle producer certifies local zero engagement without requiring
globally empty stock. It lifts the current Epeck line/circle stock
boundary—including every one-root endpoint—into the sqrt-capable exact-region
kernel, constructs the canonical annular/disk sweep, and proves its
regularized intersection with stock is empty. For nonuniform partial circles,
`FullCircleCellAuthority2` separately binds the generic partition, stock
...
validates the native trace and deciding-authority digests, and maps the three
outcomes without fallback:

- certified motion returns an immutable `MotionWitness`;
- proved exceedance raises `EngagementCapExceededError`; and
- incomplete proof raises `UnresolvedMotionEventError`.

The witness constructor is bypass-safe under direct construction and
`dataclasses.replace`: semantic kind/motion mismatch and
`effective_cap > user_cap` fail again. `InputIdentity` binds both the Python
motion-certificate schema and the native event-oracle component version.

The complete proof mechanics, reversal identity rule, exact pruning predicate,
and verification gates are documented in
[Exact Continuous Engagement](continuous_engagement.md).
...
`evaluate_exact_segment_containment(...)` constructs the exact capsule and
proves the capsule is a subset of `D`; endpoint membership in the regularized
`C_r` is retained as diagnostic evidence, not a second acceptance gate.
`evaluate_exact_full_circle_containment(...)` constructs the
...
!!! tip "Captured insight: a valid center locus may be lower-dimensional"

    In a corridor whose width is exactly one tool diameter, the valid segment
    center locus is one-dimensional. Regularized two-dimensional erosion can
    therefore represent `C_r` as empty even though the complete exact capsule
    is a subset of `D`. Requiring both endpoint anchors in that area-valued set
    produced a conservative false rejection. Containment strategy v2 makes
    `capsule subset D` the deciding predicate and preserves the anchors only as
    diagnostics. This is an exact dimensional-stratum issue, not a reason for
    an epsilon or tolerance.
...
`CoverageLedger` seeds its independent reachable-material residual with that
same authenticated center and `EntryRadius`. Its later segment/full-circle
sweeps retain `ToolRadius`. `coverage-certificate-v2` makes this distinction
part of the canonical proof schema; replay cannot relabel the larger qualified
bore radius as a cutter radius merely because both are positive lengths.
...
### Fresh candidate and state reconstruction

Task 11A now starts from canonical pocket inputs, rebuilds `ReachableDomain`
and `InputIdentity`, then rebuilds `MedialAxis` with the input-bound
`MatSamplingPolicy` and classifies a fresh `NeckInventory`. The fresh MAT
center-domain digest must equal the reachable-domain certificate before any
...
`NeckPolicy`. A foreign owner, reused transition, or changed evidence digest,
width class, or effective cap raises `ReplayEffectiveCapError`.

Replay materializes the complete candidate lattice with that reconstructed
scope/cap. The motion, neck scope, cap decision, and traversal decision must
identify exactly one candidate. Only then does replay install the passage's
next immutable state; rejected enumeration cannot consume it. An intermediate
winner becomes a `DerivedCandidateCursor`; a native-limit winner retains the
native sample. Changing only the recorded cursor-after digest fails with
`ReplayCandidateError`.

An advancing segment follows a separate reconstruction lane. Replay requires
exactly one typed `MatZeroGuideRun` on the recorded fresh edge and exactly one
native owner record with the same edge and certificate bytes. It enumerates
only `ZeroGuideLinkCandidate` records across the authenticated directed window.
Exactly one candidate must reproduce the target, neck scope, cap decision,
traversal decision, complete `AdvanceSegmentOperation` bytes, and fresh proof
record. A missing record, foreign same-edge record, changed endpoint or edge,
or two matching candidates raises `ReplayZeroGuideCandidateError` before stock
reconstruction.

### Lateral pairing is a relational proof

A `LinkSegmentOperation` is not matched as a second geometric candidate. It
transports the current exact phase to the immediately following candidate
circle while preserving that circle's cursor-before. Replay first reconstructs
the complete circle stream, then preflights every lateral operation before
constructing fresh stock:

- a link must be immediately followed by one reconstructed circle;
- its endpoint must equal that circle's exact phase;
- its neck scope and effective-cap decision must equal the circle candidate's;
- its `HoldTraversalDecision` must name the same component, edge, branch, and
  cursor-before; and
- only the following circle's `AdvanceTraversalDecision` may change the cursor.

Dangling links, consecutive links, and foreign hold cursors raise
`ReplayPairingError` before entry depletion. A map from canonical operation
ordinal to the freshly reconstructed circle candidate then authenticates both
members without inventing a second candidate type or recovering ownership from
coordinates.

An `AdvanceSegmentOperation` has no pending link and maps one-to-one to its
reconstructed `ZeroGuideLinkCandidate`. Relabelling an ordinary hold link as
an advance demands a zero-guide proof its edge does not own. Relabelling a
proved advance as a hold erases the serialized advancing role and leaves a
dangling relational pair, so grammar preflight raises `ReplayPairingError`.
The resulting `dict[int, TraversalCandidate]` is therefore a grammar ownership
map, not a coordinate association.

!!! tip "Captured insight: equal geometry does not imply equal causality"

    Hold links and advancing segments both carry segment geometry, cut depth,
    scope, and cap. Only their proof role distinguishes them: one preserves a
    cursor for a following circle; the other owns the cursor advance and the
    swept-prefix theorem. That role must be reconstructed from exact MAT proof
    and operation grammar.

!!! tip "Captured insight: token legality is weaker than sequence legality"

    Replay parses the complete lateral grammar before candidate enumeration.
    An advance before the entry circle and an unpaired later circle both use
    valid operation tokens, but neither can have the required causal owner.
    Rejecting the sequence first prevents structural corruption from surfacing
    later as an unrelated continuity or geometry error.

After the unique candidate is reconstructed, replay creates pristine
`Stock2Area` and `CoverageLedger` owners from canonical input. It applies the
authenticated `PreclearedEntry` first, proves the first circle is contained in
that void and in `D`, snapshots the post-entry/pre-circle stock for
`MotionCertifier`, and then enforces:

```text
certify motion -> deplete motion -> add exact coverage sweep
```

The adopted L fixture previously stopped at the first-circle certifier because
remote stock remained and the native circle oracle only recognized a clear
uniform rim when the complete stock was empty. Exact sweep/stock disjointness
now certifies that first circle while preserving the material elsewhere. A
chronology regression wraps the real stock, certifier, and coverage owners and
requires the complete sequence
`entry-deplete, certify, motion-deplete, coverage-sweep`.

The derived-cursor regression extends this chronology through a real direct
link:

```text
entry depletion
-> first-circle certify -> deplete -> coverage
-> link certify -> deplete -> coverage
-> second-circle certify -> deplete -> coverage
-> reject nonterminal traversal
```

The second circle must bind a decision-authority digest distinct from its
generic event-partition digest before either mutation. The terminal failure is
now traversal state, not motion certification: remaining MAT edges prevent a
partial replay certificate.

The Task 13F route-1 prefix extends the same chronology with one advancing
segment. Fresh replay reconstructs its rank-1 spatial candidate from the
independent MAT, then calls the shared `evaluate_advancing_segment_trial(...)`
pipeline on independent owners:
...
The resulting final `ReplayLateralWitness` is byte-identical to the live
zero-guide transaction. This closes advancing-segment reconstruction, not the
terminal seal: untouched routes and nonempty reachable residual still prevent
`ReplayCertificate` construction.

### Ordered evidence is a relational proof

Fresh replay now retains every proof producer's returned value instead of
discarding it after mutation. `MotionWitness` owns one versioned canonical
record and SHA-256 identity over its operation ordinal/kind, exact motion,
user/effective caps, native strategy, pre-motion stock lineage, event-trace
digest, verdict, and event counts. Replay therefore consumes the certifier's
serialization authority rather than inventing another motion-witness encoding.

`ReplayLateralWitness` then binds one recorded lateral operation to the
independently recomputed cap decision, pre-cut stock-boundary digest, exact
containment certificate, `MotionWitness`, `DepletionWitness`, and
`SweepWitness`. Construction rejects a valid proof borrowed from another
operation, kind, motion, tool radius, cap, or ordinal. This grouping is
stronger than several parallel witness arrays: equal lengths cannot prove that
their elements belong together.

The short-lived `FreshReplayTrace` separately anchors the pristine and
post-entry stock identities, the first-and-once entry depletion, the
first-circle-in-entry certificate, initial coverage, ordered lateral bundles,
and terminal stock/coverage snapshots. It recomputes both parent lineages,
requires canonical operation ordinals `2..n`, preserves the unchanged
reachable/unreachable coverage authority, and is byte-identical across two
independent rebuilds of the real circle-link-circle prefix. Canonical bytes,
not Python owner equality, compare those fresh reconstructions.

!!! note "A fresh trace is deliberately not a replay certificate"

    `FreshReplayTrace` may be constructed for the evidence that preceded a
    fail-closed terminal gate. `ReplayCertificate` may be returned only after
    every required MAT cursor is terminal and exact coverage proves the
    reachable residual empty. Keeping the types distinct prevents a diagnostic
    prefix from acquiring a completion verdict by implication.

!!! danger "Submitted neck scope is not yet causal replay evidence"

    The current candidate lattice binds a supplied `NeckScope`; it does not
    derive that scope from global MAT traversal. The same exact L-pocket
    motion and byte-identical traversal decision can therefore be enumerated
    either with `NoNeckScope` and the `120 degrees` user cap or with an
    oriented passage and its `90 degrees` cap. Fresh replay can reject a
    foreign owner, stale passage state, or changed cap after an oriented scope
    is submitted, but it cannot yet prove that this motion crossed that
    separator in that orientation.

    An edge-membership shortcut is invalid. The reproduced candidate edge lies
    in both certified cut unions and is neither plateau's complement. Causal
    scope requires Task 13's global side/branch history across the separating
    cuts. Until that state assigns the scope, terminal replay sealing remains
    disabled; the current nonterminal gate prevents this incompleteness from
    becoming a `ReplayCertificate`.

!!! warning "Passage legality is not motion feasibility"

    The real two-circle neck lineage advances legally from `unvisited` to
    `first_pass_complete`, but its second circle exceeds the production
    policy's `80 degrees` cap against post-link stock. Replay preserves that
    exact negative. A separate equal-cap fixture isolates and gates the two
    state advances, while a real first-circle test proves that the certifier
    receives the reconstructed `90 degrees` cap rather than the `120 degrees`
    user cap. Task 12 now rejects the complete joint trial atomically; Task 13
    must continue the ordered search. Neither replay nor the neck state machine
    may reinterpret cap exceedance as acceptance.

!!! danger "Certify-after-deplete can manufacture safety"

    Depleting a submitted motion before engagement certification changes the
    fact being proved: the certifier observes material that the candidate has
    already removed. Replay therefore snapshots authoritative stock before
    each lateral mutation. Coverage is updated only after the same motion has
    both certified and depleted successfully.

!!! warning "Candidate replay is not artifact certification"

    The current slice intentionally raises before returning a
    `ReplayCertificate`. Ordered fresh evidence capture is now canonical and
    cross-validated. Task 13B now supplies globally derived neck scope, but the
    replay prefix does not yet consume that authority. Integrated terminal
    traversal, exact empty reachable residual, and immutable
    replay-certificate assembly remain mandatory. The accepted no-neck and
...
| Gouge containment | stronger exact contract; finite global-family integration gated, complete generation incomplete | Gouge freedom follows from clearance-disk machining circles and offset-curve transition elements after the pocket is transformed to an assumed machinable `r + ε` geometry | One exact native kernel proves each segment capsule or circular annulus/disk is a subset of `D`, records owned `C_r` anchor facts as diagnostics, and emits an input-replayable structural record. Exact tangency, reflex crossings, island contact, disk-sweep circles, one-binary64-quantum gouges, and a lower-dimensional exact-width corridor are gated. Task 11A replays this proof for a reconstructed L-pocket prefix. Task 12 applies it to each isolated direct-link/full-circle trial. Task 13D separately requires the first complete circle sweep to lie both in the authenticated entry void and in `D`; shrinking only the entry disk rejects the launch without changing authoritative state. Task 13E runs the same exact containment gates inside one globally authenticated, invariant-ordered forward-window family. Zero-guide Task 5 applies capsule-subset containment to the real width-2 arm before any trial mutation, and Task 6 commits that proof through global dispatch. Task 8 proves all 56 direct links from the completed horizontal leaf to route 2 gouge, while 30 route-2 circles remain individually contained; exact inter-route retrace and terminal proof remain pending |
...
| Replay identity | stronger bounded-fixture contract; launch-rooted cross-axis continuation gated, terminal certificate pending | The published algorithm consumes its MAT and updates one ordered machined contour; no exact topology/provenance replay certificate is reported | Stable site, node, dual, algebraic-root, component, and coincident endpoint-event identities project once into deterministic integer rows. Native MAT and neck records reject mutation, truncation, digest mismatch, input-order drift, detached-array mutation, and omitted-cap identity. The native zero-guide inventory additionally binds each complete constant profile to the MAT/domain/tool identity and rejects missing, duplicate, mutated, or foreign records under MAT replay. Its typed Python projection retains the exact edge, MAT digest, native record bytes, and owner under one bypass-safe `MatProof`; edge, byte, digest, and owner cross-wires fail named. `EventTrace2` v2 hashes the complete deciding authority; for nonuniform circles the cell-authority digest is deliberately distinct from the generic partition digest. `MotionWitness` owns canonical bytes and a content digest over that trace plus observed stock lineage. Task 11 adds the complete `InputIdentity`; Task 11A independently rebuilds reachable domain/MAT sampling and the exact neck inventory and emits deterministic `FreshReplayTrace` bundles. Task 12 adds content-addressed physical state and atomic candidate transactions. Task 13B separately binds the complete refinement-dependent sample inventory, deterministic directed route, every edge cursor and visited incidence, and the causal neck-side frontier under one traversal digest. Task 13C uniquely reconstructs reverse candidate lineage. Task 13D binds pristine stock, entry depletion, empty coverage, first-circle proofs, global traversal parent/child, and physical child digest in one independently reproduced transaction. Task 13E adds `TraversalCommit` over physical and global parents/children and a launch-rooted `GenerationContinuation` over the ordered commit prefix. Zero-guide Task 7 independently reconstructs the advancing operation from fresh native proof bytes and reproduces its physical witness. Exact empty residual and fresh terminal `GenerationResult` remain open |
| Engagement limit | stronger exact ordered-search and atomic operation contract; complete loop still incomplete | Analytic circle construction followed by bisection until `θmax − 0.001 <= θ <= θmax` radians | Exact rational chord surrogate and event-exact segment/full-circle partition. Segment verdicts bind ordered active branches and exact pair-cap dispositions. `event-exact-motion-oracle-v5` maps each nonuniform circle cell to an exact rational station and reuses the same stationary branch/pair theorem. Source v4 admits shared-radical one-root trim vertices: an integer norm discovers roots, the original radical equation filters physical endpoint events, and exact radical sign evaluation selects the active sheet. Same-support runs bind their support-cap factor; noncoincident cross-support runs bind only topology-active pair-orientation and sub-`pi` cap resultants after exact circle-chart-pole saturation. Missing factors remain unresolved and request identities are replay-bound. Both 120-degree and 180-degree regressions retain a proved 240-degree mixed-support material arc. The isolated post-capsule positive control proves its named cap exceedance. In the real Task 13F family, exact set-order classification instead accepts trial 4: its phase seam has incomparable left/right active sheets plus tangent endpoint incidences inactive on both sides. The independently committed transaction binds the reproduced certified trace. Certified, exceeded, and unresolved outcomes remain distinct. Full continuation reaches two radius-1 S–S arms proved zero-guide by complete exact polynomial identity, carried by typed owner-bound records, enumerated as finite spatial-only links, and representable by a distinct advancing-segment operation. Task 5 certifies the real arm with a separate exact swept-prefix theorem: a clear start confines possible engagement to the closed forward semicircle and proves exactly `pi`; uncleared or tighter-cap requests stay unresolved. Task 6 dispatches the owned route-1 family and independently commits its first exact advancing segment. Task 7 fresh replay repeats the same swept-prefix theorem on independent owners and reproduces the committed witness. Task 8 localizes the next failure to missing inter-route retrace rather than cap or circle certification; remaining generation and terminal proof remain pending |
| Candidate spacing | stronger bounded-fixture finite-search contract; terminal end-to-end path incomplete | Bisection along the middle curve | All nine adopted L edges emit exact-identity line/parabola stations with independent reporting-only spacing and sagitta refinement, deterministic CSR offsets, and structural equidistant/no-site-closer verdicts. Task 10 enumerates every declared dyadic spatial/radius lane, both exact generator sites, and every finite phase; re-evaluates P–S refinement on the focus/directrix parabola rather than its sample chord; merges duplicate values without losing level provenance; performs no feasibility bisection; and matches an independent exhaustive 72-cell oracle with byte-identical repeat order. Task 3 adds the exact-proof-only link family: each radius-1 arm emits 12 spatial records in either direction with no site/radius/phase product, and the shared order ranks an available positive circle ahead of an equal-progress link. Task 12 evaluates circle candidates transactionally; Task 5 does the same for a proved advancing segment. Task 13B binds the native sample/cursor inventory to a deterministic route; Task 13C preserves the same lattice in either ordinal direction. Task 13E materializes each directed forward circle span exactly once, globally orders its family by progress/radius/identity, stops at its first accepted transaction, rejects reordered/duplicated families, and reports canonical exhaustion counts. Task 6 selects the family variant from exact MAT proof; its route-1 window contains 36 spatial records and accepts rank 1 after one dispatch. Intermediate native limits, derived interior cursors, native endpoints, and last-positive-cell exhausted cursors remain distinct proof states; terminal coverage remains pending |
| Machined state | stronger atomic launch, cross-axis continuation, and closed physical grammar; terminal coverage incomplete | Ordered contour of prior machining disks; transition sweeps omitted from that contour model | Exact stock mutation and exact full-sweep coverage use ordered certify-before-deplete lineage. Task 11 adds the physically qualified precleared disk as a distinct first-and-once depletion witness; `coverage-certificate-v2` seeds the residual with its authenticated `EntryRadius` while lateral sweeps retain `ToolRadius`. Task 11A reconstructs fresh owners and cross-validates each proof plus initial/terminal snapshots. Task 12 makes one circle continuation atomic. Task 13D closes the distinct launch. Task 13E independently replays each selected link/circle pair, advances exactly one global cursor, cross-binds both physical/global parents and children, and chains those commits under the launch transaction. Zero-guide Task 4 adds a distinct advancing segment and admits `entry circle ((hold link, circle) | advancing segment)*` while replaying phase, final cursor, cut depth, stock/coverage lineage, and oriented passage state. Task 5 makes that one-witness segment transaction atomic by evaluating on stock/coverage forks and requiring byte-identical independent commit replay. Task 6 carries the same one-witness record through global dispatch and `TraversalCommit`. Task 7 reconstructs that operation against fresh MAT, stock, and coverage owners before the deliberate nonterminal gate; exact empty reachable residual remains pending |
...
| End-to-end evidence | weaker | Complete paths, figures, path-length gains, and 3–100 ms path-generation timings excluding Voronoi construction | Tasks 6–8 and 10–13E provide exact local engagement, typed proposals, gouge certification, qualified entry, motion witnesses, input identity, fresh-prefix replay, atomic continuation, exact neck partitions, a content-addressed global traversal ledger, bidirectional finite-lattice consumption, an independently committed entry launch, and launch-rooted global continuation mechanics. The adopted L continuation stops fail-closed at an unresolved exact segment partition. Task 13F advances further: the real ordered family proves three gouges, selects trial 4, independently commits its link/circle pair, and terminalizes the first route with a trace-authenticated mixed one-root seam. The next width-2 arm has clearance exactly equal to the radius-1 tool, so exact MAT proof selects a finite spatial-only family and global dispatch atomically commits its rank-1 advancing segment without inventing a circle. Fresh replay independently reconstructs that segment and its exact witness. Full continuation now exposes route 2 at a reproducible 56-gouge direct-link boundary; reversing accepted route-1 lineage restores six containment-valid pairs, but that retrace is not yet an operation or transaction. No complete certified path, tri-dexel replay, Fig. 5 reproduction, or matched performance result exists yet |
...
Task 13B likewise removes three obvious traversal-construction pathologies
before benchmarking. One `TraversalSampleIndex` pass builds both edge and
cursor lookup views; graph projection no longer rescans all `S` samples for
each of `E` edges, and accepted-candidate limit lookup is no longer an `O(S)`
scan. Component buckets and node adjacency each assemble in one edge pass
before deterministic ordering, rather than rescanning all edges per component
or node. Authority construction derives the graph, route, and causal neck
frontiers exactly once, and every immutable child reuses that same authority.
...
most `W` limits. Task 13E supplies the production global route direction from
the start; fresh terminal replay must still consume it in the seal stage.
These are bounded structural counts, not a runtime comparison.

Task 13E now supplies that global direction. At one active cursor it selects
at most `W` native limits, calls the finite candidate producer exactly once
per resulting span, combines the immutable cells once, and visits them in
invariant order until acceptance or an authority failure. It neither rebuilds
the MAT nor rematches candidate coordinates. A rejected trial remains private;
an accepted winner is independently replayed once during commit. Reaching an
intermediate native limit retains its indexed `MatSample`, avoiding an
`O(S)` rematch before the next window.

The real scaled-L generator gate currently demonstrates the opposite of a
speed claim. Its four-test module completed in `20.45 s` on the local Release
worktree and was dominated by the ordered real-family test, which aborts on a
high-ranked link whose native exact fibre multiplicity is incomplete. This is
a development-gate wall clock, not a warmed isolated benchmark, and it is not
comparable to Held and Pfeiffer's complete planner timing. It does locate the
current cost/capability boundary inside exact segment event construction,
rather than family ordering or MAT lookup. The next performance step is a
bounded stage counter/timer on the tractable complete fixture; heuristic
candidate pruning or searching past the unresolved event is forbidden.

The Task 13F radius-1 probe now closes that integration boundary. Its qualified
point-site launch produces 16 ordered continuation candidates. Trials 1–3 are
proved gouges; trial 4 is the first feasible winner and is independently
replayed during the two-axis commit. The post-link circle contains a phase seam
where both active-sheet differences are nonempty, while exact endpoint
incidences remain outside their union. The exhaustive set relation proves a
mixed transition; requiring the active union to exhaust every incidence would
incorrectly reject a tangent that is inactive on both neighboring cells. The
reconstructed eight-cell trace digest equals the digest retained by the
accepted motion witness.

That commit terminalizes the first MAT route. The next route is a straight
width-2 arm: every sampled clearance is exactly the radius-1 tool and every
reported guide radius is zero. The positive-radius circle lattice correctly
emits no candidate. The exact zero-guide proof instead produces `12`
spatial-only candidates per arm and Task 4 gives their physical segment a
distinct advancing operation plus a closed state chronology. Global dispatch
now authenticates the active edge's retained zero-guide record before selecting
the spatial-only variant. The Task 13F forward window materializes `36`
invariant-ordered records, evaluates and accepts rank `1`, and independently
commits exactly one advancing operation (`5 -> 6`). The bounded warm
dispatch/evaluate/commit observation took `0.133066 s`; it is an internal
structural datum, not a Held–Pfeiffer comparison. A zero-radius circle,
tolerance promotion, mixed candidate family, or traversal-only terminalization
would each weaken the physical contract. The older 16-trial diagnostic took
about 61 s and included repeated independent candidate reconstruction; it was
not a planner benchmark.
...
ledger; a persistent or Merkle representation is measurement-gated until the
end-to-end generator can provide representative counts. Held and Pfeiffer
therefore retain the complete-path runtime-evidence advantage.
...
Task 10 deliberately pays for complete candidate enumeration. For each unique
spatial station and generator site, runtime is proportional to the number of
unique retained radius values times the declared phase count, plus canonical
identity serialization and final ordering. Duplicate values shared by
refinement levels are represented once with their complete level provenance;
the implementation never repeats a geometric candidate merely to remember
which lane produced it. The 72-cell golden is a completeness contract, not a
runtime benchmark. No timing yet supports parity for this new layer, and its
exhaustive set may cost more proposal work than Held and Pfeiffer's
...
Task 12 keeps that performance question observable. Every candidate trial
forks the same authoritative parent, evaluates one link and then one circle,
and discards the fork on failure. Accepted transactions contain immutable
evidence rather than a second mutable stock owner. Commit repeats exactly one
winning trial, so the correctness baseline adds one candidate evaluation per
accepted step, not one replay per rejected candidate. Candidate families and
their `MedialAxis` owner are materialized once; reconstructing a geometrically
equal MAT per trial would both violate cursor provenance and repeat expensive
exact construction.

One exploratory development probe over a 56-cell terminal candidate family
ran for more than 60 seconds and was terminated before completion. It was not
run under the benchmark harness and establishes no throughput number. It does
show that the distribution of exact event-certification cost—not Python
candidate ordering—can dominate a naive exhaustive campaign. Task 13 must
record per-stage candidate counts, rejection dispositions, event-cell counts,
and wall clocks before changing the structurally simple path. Copy-on-write
owners, cached immutable pre-state views, or policy-order early termination
are measurement-gated options; sampled engagement, heuristic pruning, and a
parallel mutable stock authority are not.

The Task-6 segment oracle now prunes a circular support only after exact
rational segment-distance arithmetic proves the support center remains farther
than the sum of support and tool radii. This happens before the fixed
per-feature-pair event grammar schedules its quadratic resultant set. In the
recovery run, the same 13-test command moved from more than 176 seconds
unfinished to 39.42 seconds complete after the combined tangent-root and
reach-pruning repair: a greater-than-4.4x observed lower bound for that repair.
This is encouraging structural evidence, but it is not an isolated oracle
benchmark and cannot be compared with Held and Pfeiffer's complete-path clock.
Line supports are currently retained unconditionally, so further pruning is a
measurement-gated opportunity rather than an assumed optimization.
...
median), including repeated production-L MAT and Task 3 domain construction,
cross-language field-19 identity, 124,796-byte field-20 construction/replay,
mutation and refinement-invariance gates, exact policy-width classification,
and synthetic sampling contracts. That is a contract gate rather than one
pocket generation. The
...
!!! warning "Freeze root identity when an endpoint enters the graph"

    `CGAL::Algebraic_real_1` is exact, but its internal defining polynomial is
    mutable: an exact equality comparison can teach a quadratic-root instance
    the equivalent linear factor of the same rational value. Recomputing
    `AlgebraicRootIdV1` afterwards can therefore change factor bytes and root
    ordinal even though the number did not change. `exact_graph_endpoint_binding`
    freezes the factor-plus-ordinal identity once; graph validation, clearance
    profiles, neck evidence, sampling, and certificate encoding consume
    `mat_endpoint_root_identity_v1` instead of interrogating the mutable CGAL
    representation. That accessor reconstructs the frozen factor/ordinal root
    and requires exact equality with the current endpoint before returning its
    ID. The native `nextafter(1, 0)` regression forces quadratic-to-linear
    refinement while a separate parameter-substitution mutation must fail.
...
| `sample_centers[N,3]`, `sample_parameter[N]` | world-XY reporting coordinates and reporting parameter; never exact evidence |
...
real signature. The tuple deliberately remains a low-level projection rather
than becoming a second result hierarchy. Task 10's
`SegmentSiteMedialAxis.build(...)` retains the proof owner behind that
projection, and `MedialAxis.build(...)` validates cross-array semantics before
creating planner-facing typed domain objects.
...
The L-pocket contains one point/point dual, between `P2 = (6,2)` and
`P4 = (2,6)`. Their rational perpendicular-bisector chart is
...
The live adaptor primitive is not the complete line `x = y`. It is a ray with
the finite source `(6,6)`, hence exact parameter `t = -1/2`, directed toward
decreasing `t`:
...
`bind_point_point_ray_parameterization` resolves both defining points through
the catalog index, requires an exact live point/point generator match, requires
exactly one finite adaptor endpoint, and requires `SDG::primal()` to return a
`Ray_2` whose source equals that endpoint. It then proves exact squared-distance
equality to both generators, converts the finite endpoint to rational
coordinates without approximation, and proves that the canonical direction is
collinear with the live ray. The sign of their exact dot product chooses the
lower or upper parameter bound.

!!! warning "Never clip the supporting line in place of the live ray"

    The full line `x = y` crosses the pocket and would manufacture a plausible
    diagonal MAT branch. The authenticated half-ray begins at `(6,6)` and runs
    outward; exact polygon clipping at `r² = 0` returns no component. Primitive
    kind and orientation are therefore topology, not rendering metadata.
...
Necks are not simply low-clearance samples. The public certificate will carry
...
### The adopted L graph now carries exact profiles

`canonical_l_shape_mat_clearance_graph` is an additive production path beside
the established graph API. It invokes the proven L graph producer once, then
resolves every final edge's two defining sites through the canonical sorted
site table:

- parallel S–S edges reconstruct their constant support distance and recheck
  its zero/negative radius-boundary signs through the existing clip contract;
- nonparallel S–S edges use the already-proved canonical
  `n₂ det(n₁,n₂)² t²` clearance law;
- P–S edges reconstruct the rational source-parabola coordinates, square the
  distance to the focus, and require its primitive integer polynomial to equal
  the existing clipping boundary at `r² = 0`.

No opaque identity is parsed, and no second Delaunay/Voronoi graph is built.
The resulting `MatClearanceProfileGraph2` requires exactly one profile per
canonical edge, in the same strict order, with identical defining sites and
complete endpoint records. The L golden at `r² = 0` is exactly:

| Profile kind | Count | Exact base function |
| --- | ---: | --- |
| Parallel S–S | 2 | `clearance²(t) = 1` |
| Nonparallel S–S | 5 | `clearance²(t) = t²` |
| P–S | 2 | rational quartic |

Complete graph-table bytes remain equal to the existing producer. Canonical
input reversal preserves all nine profile functions and endpoint records. At
`r² = 1`, every emitted clearance-root endpoint independently evaluates
through its base profile to exact `width² = 4`.

!!! note "The L restriction is not a strict-edge minimum"

    None of the nine `r² = 0` L profiles has a negative-to-positive derivative
    change in its relative interior. The two arm restrictions are
    constant-clearance edges, and other candidate minima meet graph endpoints.
    A strict-root-only extractor would therefore report no neck on precisely
    the fixture intended to exercise a narrow passage. Plateau and
    shared-vertex evidence are required semantics, not degenerate cleanup.

### Exact graph-level neck classification

`exact_neck_evidence` consumes the already-bound graph/profile bundle and
builds the topology index once. It computes exact inward clearance behavior
for both ends of every edge, validates that all incident profiles agree on the
exact algebraic width at each node, and then applies the ownership order:

1. relative-interior negative-to-positive roots remain strict-edge owners;
2. equal one-sided endpoint minima at one node merge into one shared-vertex
   owner carrying every minimizing halfedge;
3. connected constant-clearance edges merge into one maximal plateau and
   reserve their nodes against duplicate endpoint evidence;
4. one remaining one-sided minimum is retained only when its endpoint carries
   exact clearance-root evidence.

Lower and upper endpoints are normalized to one inward sign convention:
`POSITIVE` always means clearance rises when entering the edge. The sign is
evaluated at a rational witness in the first exact derivative-root-free open
cell, so a function such as `t⁴` is correctly increasing away from `t = 0`
even though its derivative equals zero at the endpoint.

The internal `MatExactNeckEvidence2` is a closed `std::variant`, so a payload
cannot disagree with its type tag. Every record binds the deterministic owner,
sorted defining-site union, exact algebraic squared width, and independently
proved separating cut. A nonseparating candidate is omitted only through the
named `NonSeparatingMatNeckCandidateError`; malformed graph, profile, width,
or plateau contracts still fail loudly.

!!! note "Constant-cell ownership is not an exit-slope test"

    Every relative-interior point of a constant profile is a non-strict local
    minimum on that MAT cell. Maximal connected constant cells therefore merge
    regardless of whether an adjacent terminal branch rises or falls. The
    independent contracted-plateau cut rejects topologically irrelevant flat
    cells. Requiring every exit to rise would incorrectly erase a
    constant-width passage that connects toward a narrowing terminal branch.

On the adopted L graph, the two constant profiles become exactly two
disjoint, single-edge `PLATEAU` records. Each has exact `width² = 4`, two
plateau node IDs, one plateau edge ID, a canonical owner, the two defining
sites, and at least two traversal-side partitions. Canonical input reversal
...
The production adapter also seals exact sample authority into the bundle. Its
private verified path records the exact clearance-radius square and attaches
the two structural verdicts to every station:

1. the station is equidistant to its two generators because its exact
   parameter identity lies on the source-derived, edge-authenticated bisector
   chart;
2. no catalog site is closer because the station remains inside the exact
   retained interval of the source Voronoi halfedge.

`numeric_sample_table(...)` requires both verdicts and an exact match between
the bundle's rational clearance bound and the typed binary64 tool radius
before it emits fields 10–15. It then evaluates reporting clearance from the
edge profile and reports the one-sided MATHSM guide radius `(d-r)/2`.

!!! warning "A valid record shape does not grant proof authority"

    `MatProposalSamplingGraph2::build(...)` can reconstruct a structurally
    well-formed bundle for testing, but it deliberately carries no clearance
    bound and zero exact verdicts. Numeric export rejects it. Only the exact
...
### Field 19 reuses Task 3 identity

`ReachableDomain2::build(CanonicalReachInput2)` is an additive factory for
consumers that already own canonical rings and radius. It preserves the raw
matrix constructor while avoiding a second input-normalization pass. The MAT
compositor builds the exact Task 3 domain once, then
`reachable_domain_certificate_identity(...)` encodes the proof-owning domain
with the same frozen CCAN grammar as Python:

- big-endian finite binary64 with signed zero normalized;
- canonical world-XY points, outer ring, sorted hole rings, and polygon;
- exact source, arrangement-vertex, selected-cell, and component records;
- all three Task 3 proof verdicts and its strategy version;
- SHA-256 certificate and `exact-region-center-domain-v1` identities.

The radius-`0.5` L golden is cross-language locked to 48,947 certificate bytes,
certificate digest
`77b7b15003f2d5de9fbee7a495d4d49004d958a58d10a9c73a9265e3a98d5874`,
and center-domain digest
`98aaa87a6fdc0589ef364e7ae3edad561208ff8d4245b340d616c63878be12de`.
Canonical input reversal preserves all three.

!!! warning "Proof ownership precedes identity encoding"

    The identity encoder accepts `ReachableDomain2`, not a public
    `ReachableDomainCertificate2` aggregate. Sorted, nonempty record vectors
    establish a valid shape but cannot prove that no exact record was deleted.
    Only Task 3's exact domain builder owns that authority; replay must rebuild
    it and compare the resulting digest.

### Field 20 seals the refinement-invariant exact branch

`certified_mat_exact_projection_v1(...)` is the sole certificate factory. It
computes the validated graph table and exact neck projection once, then uses
those same owned values to emit `segment-site-mat-certificate-v1`. Production
does not rebuild the MAT, rematch sites, or rerun neck classification merely
to serialize proof. The completed byte record moves into field 20 at the final
ownership boundary; it is not copied.

The certificate binds:

- field 19's 32-byte center-domain digest;
- canonical site identities, typed provenance, and segment endpoint owners;
- exact node records and edge records, including algebraic endpoint root IDs,
  coincident event evidence, defining sites, original dual, and clip component;
- every exact rational `clearance²(t)` polynomial;
- the hidden node/original-dual dictionaries and fields 2–9's complete
  deterministic integer projection;
- canonical neck evidence and fields 17–18's cut CSR;
- authenticated rejected-transition and matched-generator counts.

It deliberately excludes reporting node coordinates and fields 10–15.
Changing station spacing from `0.75` to `0.25` changes sample cardinality but
preserves certificate bytes. The radius-`0.5` production L golden is 124,796
bytes with SHA-256
`59ccf145c161b819ee91b047edba687eb9b4f0a697a4ed49574a8692f3e986dd`.
Canonical input reversal is byte-identical. Exact replay rebuilds the
projection and rejects a changed byte, truncation, a non-32-byte domain
digest, or a different valid 32-byte domain digest.

!!! warning "The certificate is exact MAT identity, not proposal identity"

    Sample coordinates, reported clearance, guide radius, and sample
    parameters are replaceable refinements over one exact MAT. Binding them
    into field 20 would make a harmless rendering or candidate-density change
    look like a topology change and would contradict the proof-boundary
    dataflow.
...
| `segment_site_mat_bundle.*` | single proof-owning MAT/catalog/domain build, fixed-table projection, exact identity dictionaries, and certificate-first neck classification |
| `segment_site_mat_numeric_table.*` | projection compositor used by the proof owner for deterministic native fields 1–20 |
| `segment_site_mat_certificate.*` | refinement-invariant exact MAT CCAN certificate, SHA-256 digest, owned graph/neck projection, and exact replay |
| `segment_site_zero_guide.*` | complete exact constant-profile inventory, canonical per-edge zero-guide records, and MAT-replayed mutation verification |
| `reachable_certificate_encoding_2.*` | proof-owned cross-language Task 3 certificate bytes, certificate digest, and center-domain digest |
| `canonical_encoding.*` | frozen native CCAN byte, integer, binary64, rational, boolean, sequence, component-map, and tagged-union encoding plus canonical rational decoding |
...
| `medial_axis_2.cpp` | conversion-only nanobind adapter, module-local error translation, detached fixed-tuple projection, and native proof-owner binding |
| `compas_cgal/_medial_axis_2.pyi` | literal low-level tuple ABI, proof-owner API, exact identity views, and named exception types |
| `compas_cgal/adaptive/medial_axis.py` | typed topology/sample/proof projection using native exact IDs without coordinate matching |
| `compas_cgal/adaptive/neck.py` | exact evidence inventory, oriented immutable passage state, and bound cap transitions |
| `compas_cgal/adaptive/neck_topology.py` | closed exact neck-locus variants and evidence-bound ordered side partitions |
| `compas_cgal/adaptive/candidates.py` | pure complete finite spatial/radius/site/phase enumeration and canonical candidate identity/order |
| `compas_cgal/adaptive/traversal_graph.py` | immutable refinement-dependent sample index, exact MAT graph projection, and deterministic directed route discovery |
| `compas_cgal/adaptive/traversal.py` | global cursor/incidence ledger, causal neck-side frontiers, and immutable route-state transitions |
...
| Exact neck classification | native records and Task 10 typed consumption gated for the L fixture | strict, clearance-endpoint, shared-vertex, and maximal plateau ownership; exact node-width consistency; independent cut attachment; canonical CCAN records and SHA-256 digests; deletion/mutation/order rejection by exact replay; adopted L emits exactly two byte-identical reversal-invariant plateau records at `width² = 4`; typed inventory binds exact owner/sites/cut |
| Exact neck width classes | native classification and typed policy consumption gated for the L fixture | certificate-first replay; cross-language `ExactRationalV1` decoding; nonnegative strictly increasing boundaries; exact algebraic comparisons; right-closed restrictive equality; evidence-digest-bound comparison certificates; both production L plateaus classify as `0` at boundary `4` with reversal-identical certificates |
...
| Native zero-guide proof inventory | implemented and native-owner/Python-projection gated for the L fixture | exact coefficient identity `coefficients(C_e) == [r²]`; two radius-1 S–S records; radius-0.5, `nextafter(1, 0)`, and P–S endpoint-contact nonmembers; canonical reversal; missing/duplicate/mutated/foreign replay rejection; typed immutable edge/MAT-digest/native-byte records; edge/byte/digest/owner cross-wire rejection; finite link candidates, advancing operation, atomic transaction, proof-directed global dispatch, and fresh advancing-segment reconstruction implemented; terminal seal pending |
...
| Typed MAT planner view | implemented and Python-gated for the production L fixture | direct native site/node/edge/dual/parameter identities; detached reporting arrays; typed components, branches, clipped endpoint state, tool-fit runs, proposal cursors, zero-guide proof inventory/read-only edge view, and retained native proof owner; no coordinate matching or float reclassification |
...
| Directed global traversal ledger | implemented and Python-gated for production L plus synthetic graph contracts | exact component/branch/node route; iterative chain/branch/cycle/multi-component coverage; one cursor and two terminal incidences per edge; deterministic causal three-side frontiers; locus deferral; ambiguous/overlapping transit rejection; one-time cold authority construction; direction-retaining forward/reverse span consumption; Task 13E now cross-binds accepted physical/global continuation |
| Atomic entry-circle bootstrap | implemented and Python-gated for one real reverse P–S L launch | content-addressed input plus global traversal parent/child; pristine stock reconstruction; first-and-once entry depletion; empty initial coverage; exact phase equality; complete sweep-in-entry and design containment; event certification against post-entry stock before depletion; circle coverage; independent byte-identical commit; no synthetic link or invented causal passage; 15 focused ownership/chronology mutations; Task 13E roots its continuation in this transaction; terminal coverage remains pending |
| Finite candidate lattice | circle and proved zero-guide paths implemented and Python-gated for adopted L-edge goldens; global variant dispatch implemented | complete merged dyadic spatial/radius lanes, both generator sites, all finite phases, MATHSM maximum-radius formula, focus/directrix parabola evaluation with chord-midpoint rejection, exact native, derived-interior, intermediate-native, and exhausted-terminal cursor states, cross-wired lineage rejection, non-aligned minimum guard, exhaustive 72-cell circle oracle, deterministic furthest/radius/identity order; each proved three-millimetre arm emits 12 spatial-only link records in either direction, with no circle-derived fields and named unproved-edge/cross-wire failures; positive radius outranks zero at equal progress; Task 13E materializes each active circle span once and names exact exhaustion; Task 13F's route-1 window materializes 36 owned zero-guide records and accepts rank 1 after one dispatch; final coverage remains pending |
| Exact gouge containment | implemented and native/Python-gated for polygonal segment/full-circle fixtures | exact segment capsule and circle annulus/disk subset of `D`; owned `C_r` anchor diagnostics; immutable native structural replay; lower-dimensional exact-width corridor; outer disk diagnostic only; equality, reflex, island, disk-sweep, exact tangency, and next-binary64-gouge mutations |
...
| Independent fresh replay | circle and advancing-segment prefixes implemented and Python-gated | canonical geometry/policy argument equality; exact approach/plunge/lateral grammar and phase continuity; material-side-derived orientation; fresh reachable-domain, MAT, zero-guide proof, and exact neck-inventory reconstruction; center-domain authentication; complete forward-window candidate enumeration; unique motion/scope/cap/traversal/proof match; native/derived cursor continuation; independent oriented passage state; exact effective-cap delivery to circle, hold-link, and swept-prefix segment certification; foreign-owner, reused-transition, missing/foreign proof, ambiguous candidate, relabelling, cap-exceeded, and traversal mutations fail loud; no certificate emitted until complete witnesses, terminal traversal, and empty residual close |
| Atomic candidate transaction | circle and zero-guide segment variants implemented and Python-gated for the production L fixture | content-addressed `GenerationState`; independent stock/coverage forks; complete passage history, link/circle pairing, advancing-segment chronology, and qualified cut-depth replay; candidate/depletion/cap/cut-direction policy binding; exact containment/TEA/depletion/coverage order; generic frozen-stock event proof for hold links and a distinct clear-start swept-prefix `pi` theorem for advancing cuts; immutable evidence; real cap/gouge failures remain atomic; stale parent/cursor rejection; independent byte-identical replay; deterministic exact-policy selection; Task 13E supplies an explicit authenticated cursor to the circle engine, while Task 5 adds the separately typed segment transaction |
| Global finite-search continuation | circle and zero-guide paths implemented and Python-gated; real completion now blocked at exact inter-route retrace and terminal coverage | launch-rooted `GenerationContinuation`; proof-directed one-time family variant selection per active edge; one family build per directed forward span; invariant first-feasible order; no lower-ranked dispatch; exact gouge/cap/degenerate-link continuation only; canonical non-neck/causal-neck/mixed exhaustion counts; independently replayed two-axis commit; deterministic completed-route activation; adopted fixture propagates an incomplete segment partition. Task 13F runs the integrated source-v4 route: exactly three gouges precede trial 4, its mixed seam retains inactive tangent incidence evidence, independent commit replay matches, and route 0 terminalizes. Route 1 is a constant tool-clearance arm with no positive circle cells; the retained exact zero-guide proof selects a 36-record spatial-only window, rank 1 is the sole evaluated candidate, and its one-witness advancing transaction independently commits. Fresh replay uniquely reconstructs that candidate and reproduces its witness. Route 2 has 56 candidates: every direct link from `(5,1)` gouges, while a contained reverse of accepted route-1 lineage restores 10 links and six complete containment-valid pairs. Transit operation/transaction, terminal residual, and replay seal remain pending |
...
The Task 10 planner boundary additionally runs:
...
The Task 12 transaction boundary additionally runs:
...
At the 2026-07-28 stage close, the focused transaction suite passed all 29
contracts and the complete adaptive suite passed all 468 tests in 224.69 s.
Strict mypy covered 22 adaptive source files, Ruff was clean, and MkDocs built
strictly. These are repository gates, not planner-performance measurements.

At the 2026-07-29 Task 13E stage close, affected selection passed `113` tests
and the complete adaptive suite passed `518` tests in `223.88 s`. Ruff, strict
mypy over `27` source files, strict MkDocs, and `git diff --check` passed. The
new gates cover branch-switch cross-binding, finite ordered search, named
exhaustion, forward-window orchestration, cursor-state distinctions,
launch-rooted continuation, and fail-closed native substrate translation.
This suite wall time is verification cost, not Held–Pfeiffer planner timing.

At the 2026-07-29 Task 13F route-0 checkpoint, the focused native-seam and
ordered-commit slice passed `4` tests in `28.22 s`, affected `--testmon`
selection passed `5` in `29.37 s`, and the complete adaptive suite passed
`523` in `245.18 s`. Ruff, strict mypy over `27` source files, strict MkDocs,
and `git diff --check` passed. The new gate authenticates three ordered gouge
rejections, the first feasible transaction, independent physical commit,
mixed phase-seam set relations, retained inactive endpoint incidences, and
native/Python trace-digest identity.

At the 2026-08-01 zero-guide Task 1 checkpoint, the complete medial-axis owner
file passed `17` tests and the complete adaptive suite passed `531` tests in
`306.41 s`. The clean official Task 9 configure/build/run gate, Ruff, strict
mypy over `27` source files, and strict MkDocs passed. Eight new owner contracts
cover the two exact radius-1 records, repeat/reversal bytes, radius-0.5 and
`nextafter(1, 0)` nonmembership, P–S endpoint-contact exclusion, and
missing/duplicate/mutated/foreign/cross-certificate replay failures. The native
gate additionally forces CGAL to refine one exact parameter from a quadratic
to an equivalent linear representation, proves its frozen identity survives,
and rejects substitution of a different exact parameter. Suite wall time is
verification cost, not planner timing.

At the 2026-08-01 zero-guide Task 2 checkpoint, the complete medial-axis file
passed `22` tests and the complete adaptive suite passed `536` in `267.28 s`.
Affected `--testmon` selection passed `7`; Ruff, strict mypy over `27` source
files, strict MkDocs, formatting, and diff checks passed. Five new typed
contracts cover the owned happy path plus edge-ID, native-record-byte,
MAT-digest, and native-owner cross-wires. The `980`-line planner module remains
below the task's `1,000`-line ceiling. Suite wall time is verification cost,
not Held–Pfeiffer planner timing.

At the 2026-08-01 zero-guide Task 3 checkpoint, the complete
candidate/policy/traversal slice passed `52` tests and the complete adaptive
suite passed `542` in `187.61 s`. Final affected `--testmon` selection passed
`7`; Ruff, strict mypy over `27` source files, strict MkDocs, formatting, and
diff checks passed. Six new contracts cover both proved arms in both
directions, deterministic 12-record spatial families, derived/native endpoint
cursors, unproved-edge and complete candidate-field rejection, zero-radius
ordering, and closed-union global traversal. `candidates.py` remains below its
hard task ceiling at `1,197` lines. Suite wall time is verification cost, not
Held–Pfeiffer planner timing.

At the 2026-08-05 zero-guide Task 5 checkpoint, the uncached exact
containment/oracle/certificate/transaction/identity slice passed `93` tests in
`42.34 s`, and the complete adaptive suite passed `562` in `224.43 s`. Ruff,
strict mypy over `28` source files, strict MkDocs, formatting, and diff checks
passed. The new contracts cover lower-dimensional exact containment, the
clear-start swept-prefix `pi` theorem, sealed native audit reconstruction,
native/Python strategy and theorem identity, generic-witness relabelling,
proof-output cross-wires, four-stage atomic failure, same-shape opaque-lineage
replay rejection, and byte-identical commit. Suite wall time is verification
cost, not Held–Pfeiffer planner timing.

At the 2026-08-05 zero-guide Task 6 checkpoint, RED exhausted an empty circle
family with `attempts=0`. The uncached generator file then passed `6` tests in
`27.77 s`, affected `--testmon` selection passed `6` in `28.52 s`, and the
complete adaptive suite passed `563` in `224.39 s`. Route 0 retains its fourth
circle winner. Route 1 authenticates the active native zero-guide record,
materializes `36` spatial candidates, evaluates and accepts rank `1`, adds one
operation (`5 -> 6`), and independently commits the cross-axis child. The
bounded warm route-1 dispatch/evaluate/commit observation was `0.133066 s`.
Adversarial mutations additionally reject direct evaluation or commit with a
foreign same-edge native record and reject a child that hides extra operations
before the expected transaction tail. `generator.py` is `1,017` lines: large
by project guidance, below the `1,200`-line suspicious threshold, and still
single-purpose. Ruff and strict mypy over `28` source files passed; suite and
file wall times remain correctness evidence, and the warm observation is not a
matched Held–Pfeiffer planner timing.

At the 2026-08-05 zero-guide Task 7 checkpoint, RED stopped at the old
hold-link/circle grammar before any physical mutation. Fresh replay now
authenticates one typed and one native zero-guide record on the recorded edge,
enumerates only the spatial family on the fresh directed window, selects one
candidate, and reproduces the live advancing-segment witness byte for byte.
The mutation matrix rejects advance-before-entry and unpaired-later-circle
sequences before candidate search. Advancing-proof mutations, changed endpoint
or edge, missing native record, foreign same-edge proof bytes, and two matching
candidate identities raise `ReplayZeroGuideCandidateError`; relabelling the
advance as a dangling hold raises `ReplayPairingError` because its proof role is
no longer encoded. The review-fixed affected `--testmon` selection passed `20`
tests in `63.97 s`; uncached replay/generator files passed `28` in `39.52 s`;
the complete adaptive suite passed `566` in `235.47 s`; Ruff, strict mypy over
`28` source files, strict MkDocs, and diff checks passed. Independent re-review
found no remaining Critical or Important issue. General replay tests remain
separate from the focused `400`-line Task 13F zero-guide replay module.
`replay.py` is `1,276` lines: suspicious but below the mandatory split boundary
and still owns one replay pipeline. Terminal traversal and exact empty residual
remain Task 8; these wall times are regression evidence, not a matched planner
result.

At the 2026-08-05 Task 8 boundary, full launch-rooted continuation reproduces
route-family counts `(16, 36, 56)`, commits the established circle and
zero-guide winners, then raises `NoFeasibleCandidateError` at route-2 cursor
`def1bf1471e2df355ba488378ffad7b9e20116ac81909d9f9822a5a62b6abbb0`.
The ordered route-2 family identity digest is
`56d92fcf2089c1e771a9add79bd356d172904a69e6c7acb18ab02522f4ed093b`.
All `56` direct links from phase `(5,1)` are exact gouges; cap and
degenerate-link counts remain zero. This is not circle-family exhaustion:
`30` circles are individually contained. The exact reverse of the accepted
route-1 segment is contained and restores `10` contained links and `6`
contained link/circle pairs from phase `(1.7480694691784169,1)`.

The route ledger is a deterministic DFS discovery order, not a continuous
physical walk. The next stage must therefore bind inter-route retrace to
accepted operation lineage instead of widening the lattice, skipping route 2,
or retracting without a new process contract. A generic exact link proof for
the reverse remained active beyond `10 min` at approximately 99% CPU before
the bounded diagnostic was terminated. The existing swept-prefix theorem
certified the submitted reverse in `0.040943 s` with two exact strata. These
diagnostic timings show why proof-class selection matters; they are not a
matched Held–Pfeiffer benchmark.
...
gate independently defines the calculus consumed by production profiles.
The catalog-neck gate then requires the adopted L bundle's graph table to
remain identical to the existing producer, locks one ordered profile per edge
and the exact `2 constant + 5 quadratic + 2 quartic` decomposition, and proves
that no profile has a relative-interior strict minimum. Complete profile
records survive canonical input reversal. The `r² = 1` gate re-evaluates every
clearance-root endpoint to exact `width² = 4`; missing, reordered, or
noncanonical graph/profile rows fail loudly. The neck-evidence gate separately
locks all four typed variants, exact open-interval endpoint signs for affine,
constant, quadratic, and zero-derivative `t⁴` behavior, exact node-width
agreement, maximal multi-edge plateau merging, ownership precedence, and
independent cut attachment. Malformed endpoint-root provenance,
...
missing/reordered/cross-edge failures. The proposal-table gate then binds the
positive exact tool radius, composes the existing topology table with numeric
fields 10–15, requires all structural sample verdicts, checks reporting
...
fail through distinct named errors. The indexed endpoint gate also binds the
L-pocket's sole P–P ray to its canonical rational chart, proves the exact
`(6,6)` live source and `t <= -1/2` orientation, and requires exact domain
