...
!!! warning "Implementation maturity"
    ...
    typed link-only planner consumption remains pending.
...
| --- | --- | --- | --- |
| Exact MAT proof | native `SegmentSiteMedialAxis` | exact graph, catalog, site/node/edge/dual/parameter identity, neck evidence, center-domain digest, and MAT-certificate replay | planner policy or mutable traversal state |
| Typed planning state | Python `MedialAxis`, `NeckInventory`, and `NeckPassage` | immutable topology views, exact identity references, verified neck class, oriented passage transition, and bound effective cap | geometric acceptance of a proposed motion |
...
- one `MatProof` that keeps the native owner, center-domain digest, and MAT
  certificate alive together.
...
replay authority. Typed Python inventory, link-only candidate/transaction,
route-1 consumption, terminal residual, and final replay remain subsequent
tasks.
...
| Native zero-guide proof inventory | implemented and native-owner/Python-binding gated for the L fixture | exact coefficient identity `coefficients(C_e) == [r²]`; two radius-1 S–S records; radius-0.5, `nextafter(1, 0)`, and P–S endpoint-contact nonmembers; canonical reversal; missing/duplicate/mutated/foreign replay rejection; typed planner projection and link-only consumption pending |
...
| Finite candidate lattice | circle path implemented and Python-gated for adopted line/P–S L-edge goldens plus global family consumption; native zero-guide authority implemented, typed link path pending | complete merged dyadic spatial/radius lanes, both generator sites, all finite phases, MATHSM maximum-radius formula, focus/directrix parabola evaluation with chord-midpoint rejection, exact native, derived-interior, intermediate-native, and exhausted-terminal cursor states, cross-wired lineage rejection, non-aligned minimum guard, exhaustive 72-cell oracle, deterministic furthest/radius/identity order; three exact-clearance L leaves terminalize at their last feasible positive-radius lattice station only after exhaustion; Task 13E materializes each active directed span once, stops at the first accepted transaction, and names exact exhaustion; Task 13F detects that a constant tool-clearance edge has no positive circle cell, and Task 1 now supplies the exact native proof record required to authorize a separately certified centerline-link family rather than an empty-family terminal shortcut; final coverage remains pending |
