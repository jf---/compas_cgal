...
and the chart-specific cap projections that keep same-support run signs
invariant over each cell. Cross-support runs remain unresolved.
`event-exact-motion-oracle-v3`, `full-circle-uniform-event-exact-v2`, and
`full-circle-cell-strata-exact-v1` make the stronger deciding procedures part
...
| Engagement limit | stronger exact ordered-search contract; complete loop blocked by an explicit unresolved substrate case | Analytic circle construction followed by bisection until `θmax − 0.001 <= θ <= θmax` radians | Exact rational chord surrogate and event-exact segment/full-circle partition. Segment verdicts bind ordered active branches and exact pair-cap dispositions. `event-exact-motion-oracle-v3` maps every nonuniform circle cell to an exact rational station, reuses the same stationary branch/pair theorem, and binds same-support material-run dispositions to chart-specific cap projections; unsupported cross-support cells remain unresolved. The uniform path also proves an exact circular sweep is disjoint from nonempty remote stock, closing zero-engagement entry recuts without sampling. Certified, exceeded, and unresolved outcomes remain distinct. Task 12 delivers the reconstructed cap to both link and circle on isolated stock and discards the pair if either fails; the production `80 degrees` second circle is a gated negative. Task 13E derives full or causal neck cap from global/physical state, continues only proved gouge/cap/degenerate-link rejection, and stops at the first feasible ordered cell. A real high-ranked L-fixture link exposes `IncompleteSegmentPartitionError`, translated to `UnresolvedMotionEventError` and propagated before later feasible cells |
...
| End-to-end evidence | weaker | Complete paths, figures, path-length gains, and 3–100 ms path-generation timings excluding Voronoi construction | Tasks 6–8 and 10–13E provide exact local engagement, typed proposals, gouge certification, qualified entry, motion witnesses, input identity, fresh-prefix replay, atomic continuation, exact neck partitions, a content-addressed global traversal ledger, bidirectional finite-lattice consumption, an independently committed entry launch, and launch-rooted global continuation mechanics. The current real L continuation stops fail-closed at an unresolved exact segment partition. No complete certified path, tri-dexel replay, Fig. 5 reproduction, or matched performance result exists yet |
...
The `event-exact-motion-oracle-v3` uniformly-clear path derives one
...
or boundary inside the cell loop. On the Apple M1 Max Release worktree, that
structural repair changed a warm 10-cell certificate median from 252 ms to
128 ms over 10 measured calls, and a 43-cell cross-support audit from 572 ms to
301 ms over 5 calls: 1.96x and 1.90x. These timings isolate one exact circle
audit and are not comparable to Held and Pfeiffer's complete planner clock.
The matched benchmark must expose uniform and nonuniform certification
separately; runtime pressure cannot justify a parallel mutable stock authority
or a sampled clear test.
