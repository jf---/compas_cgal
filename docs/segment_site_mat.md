...
!!! warning "Candidate replay is not artifact certification"
    ...
    cross-validated, but terminal traversal, exact empty reachable residual,
    and immutable replay-certificate assembly remain mandatory Task 11A work.
    The accepted no-neck and equal-cap oriented-neck prefixes deliberately end
    with `ReplayTraversalError` while untouched MAT edges remain nonterminal.
