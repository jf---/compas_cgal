...
!!! warning "Implementation maturity"
    ...
    Python now projects them into immutable MAT-digest-bound proof records.
    Typed link-only candidate and transaction consumption remains pending.
...
| Finite proposal lattice | Python `MiddleCurveSpan`, `MathsmCircleProposal`, and `MiddleCurveCandidate` | complete deterministic proposal enumeration and canonical winner order | gouge freedom, engagement, depletion, or coverage |
...
Task 2 therefore closes typed Python ownership. Link-only candidate and
transaction construction, route-1 consumption, terminal residual, and final
replay remain subsequent tasks.
...
The implementation exposes no feasibility predicate and performs no
bisection. It materializes the declared Cartesian set, then orders candidates
by:

1. furthest exact progress;
2. largest squared radius of the represented circle;
...
| Native zero-guide proof inventory | implemented and native-owner/Python-projection gated for the L fixture | exact coefficient identity `coefficients(C_e) == [r²]`; two radius-1 S–S records; radius-0.5, `nextafter(1, 0)`, and P–S endpoint-contact nonmembers; canonical reversal; missing/duplicate/mutated/foreign replay rejection; typed immutable edge/MAT-digest/native-byte records; edge/byte/digest/owner cross-wire rejection; link-only consumption pending |
...
| Finite candidate lattice | circle path implemented and Python-gated for adopted line/P–S L-edge goldens plus global family consumption; typed zero-guide authority implemented, link candidate path pending | complete merged dyadic spatial/radius lanes, both generator sites, all finite phases, MATHSM maximum-radius formula, focus/directrix parabola evaluation with chord-midpoint rejection, exact native, derived-interior, intermediate-native, and exhausted-terminal cursor states, cross-wired lineage rejection, non-aligned minimum guard, exhaustive 72-cell oracle, deterministic furthest/radius/identity order; three exact-clearance L leaves terminalize at their last feasible positive-radius lattice station only after exhaustion; Task 13E materializes each active directed span once, stops at the first accepted transaction, and names exact exhaustion; Task 13F detects that a constant tool-clearance edge has no positive circle cell, and Tasks 1–2 now supply its native proof plus typed owner-bound projection to authorize a separately certified centerline-link family rather than an empty-family terminal shortcut; final coverage remains pending |
