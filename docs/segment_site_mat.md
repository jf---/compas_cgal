...
!!! warning "Implementation maturity"
    ...
    finite spatial-only link-candidate family. Typed advancing operation and
    transaction consumption remains pending.
...
reported guide radius is zero. The current circle-only lattice correctly emits
no candidate, so full continuation fails loud with `attempts=0`. This is not a
reason to skip the route—the tool-center link performs required removal. The
next implementation stage is a separately typed, exact-MAT-certified
link-only candidate and transaction path. A zero-radius circle, tolerance
